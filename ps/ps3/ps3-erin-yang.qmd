...
```{r}
# call your function here
logistic_growth(2000, 250, 0.25, 15)
```
...
```{r}
# call your function here
for (r in c(0.15, 0.25, 0.35)){
    logistic_growth(2000, 250, r, 15)
}
```
...
```{r}
# write your function here
# binoomial probability func
# inputs: n = number of trials, k = number of successes, p = probability of success
# output: a number between 0 and 1 probability

bin_probability <- function(n, k, p){
    if(n < 1){
        stop('there must be at least 1 trial')
    }
    else if(k > n){
        stop('cant have more successes than trials')
    }
    else if(p < 0 | p > 1){
        stop('p must be a fraction')
    }
